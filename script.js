...
function loadData() {
  ...
  // Default data if cookie is absent or invalid
...
// Function to update cookie with current data
...
function displayData() {
  ...
  data.forEach((item, index) => {
    ...
    if (item.type === 'pair') {
      ...
      // Add event listener for real-time hex update and cookie storage
...
async function copyAllContent() {
  ...
  let content = contentParts[0]; // Start with first comment
  ...
  try {
    ...
    updateCookie(); // Update cookie after successful convert
  } catch (err) {
    console.error('Failed to copy:', err);
    // Attempt to request clipboard permission
    const hasPermission = await requestClipboardPermission();
    if (hasPermission) {
      try {
        await navigator.clipboard.writeText(content);
        showToast('Hex codes and comments copied to clipboard!');
        updateCookie(); // Update cookie after successful retry
      } catch (retryErr) {
        console.error('Retry failed:', retryErr);
        showToast('Failed to copy to clipboard. Please try again.', true);
      }
    } else {
      showToast('Clipboard permission denied. Please enable it in your browser settings.', true);
    }
  }
}
